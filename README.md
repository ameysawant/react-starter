...
After `npx shadcn@latest init`, shadcn adds `oklch()` color values in `global.css` by default. Replace the full `src/shared/styles/global.css` file with the hex version below.

**Correct section order in `global.css`:**
...
Do not put `body {}` between `:root` and `@theme inline`. Keep body styles inside `@layer base`.
